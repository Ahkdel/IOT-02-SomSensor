...
void vPresentaMacIp(String sMac, String sIP) {
  ...
  tft.drawString(szSSID.c_str(), 160, 180, GFXFF);// Nom de la xarxa
  tft.setFreeFont(FF31);
...
void vTryToConnectWiFi() {
  IPAddress IPtemp;
  if (bTryWifiConnection() && (WiFi.status() == WL_CONNECTED)) {
    IPtemp = WiFi.localIP();
    strcpy(sMac, szGetMac().c_str());
    Serial.println("Connected to WiFi successfully!");
    Serial.print("IP:  "); Serial.println(IPtemp.toString());
    Serial.print("MAC: "); Serial.println(sMac);
  }
  else {
    IPtemp = IPAddress(0,0,0,0);
  }
  sIP = IPtemp.toString();
}
